...
def edit(lead_id, label=""):
    ...
    if resp["API"] != "OK":
        return render_template(
            "lead.html", lead_id=lead_id, label=label, error=json.dumps(resp)
        )
    else:
...
def getLead(lead_id, picklist=False):
    ...
    try:
        ...
        cur.execute(
            "SELECT status, dsr, presales, notes, EXTRACT(epoch from updated), dq_reason, opp_number FROM aseaton.lead_status WHERE sfdc_lead_id = %s",
            (lead_id,),
        )
        row = cur.fetchone()
        ...
        if picklist:
            ...
            cur.execute(
                "SELECT pn, pv FROM aseaton.lead_params WHERE type = 'lov' ORDER BY pn, pv"
            )
            rows = cur.fetchall()
...
def setLead(lead_id):
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO aseaton.lead_status (sfdc_lead_id, status, opp_number, dq_reason, dsr, presales, notes) values (%s, %s, %s, %s, %s, %s, %s) "
        ...
        cur.execute(
            "INSERT INTO aseaton.lead_status_history (sfdc_lead_id, status, opp_number, dq_reason, dsr, presales, notes) values (%s, %s, %s, %s, %s, %s, %s) ",
            (lead_id, req["status"], opp_number, dq_reason, dsr, presales, notes),
        )
